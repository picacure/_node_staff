...
module.exports.image = function (modules, opts, callback) {
	...
	Object.keys(modules).forEach(function (id) {
		...
		//var index = -1;
		var index = -1,
			newId = id.valueOf().toString();

		index = newId.lastIndexOf('\\');
		newId = newId.substring(index,newId.length);


		nodes[id] = nodes[id] || g.addNode(newId);
		...
		modules[id].forEach(function (depId) {
			...
			index = depId.lastIndexOf('\\');
			depId = depId.substring(index,depId.length);
